        ...
        implements JdbcConnectionFactory
{
    ...
    public Connection getConnection(final JdbcCredentialProvider jdbcCredentialProvider)
    {
        try {
            ...
            LOGGER.info("connector version 2022.02.15-4");
            //kdb only
            if(databaseConnectionConfig.getType() == DatabaseEngine.KDB) {
                ...
                try {
                    com.amazonaws.athena.connectors.jdbc.kdb.KdbMetadataHandler.declareFunctionsFromS3(conn);
                } catch(IOException ex) {
                    throw new SQLException("error on declaring functions", ex);
                }
                LOGGER.info("caching schema and function list.");
                com.amazonaws.athena.connectors.jdbc.kdb.KdbMetadataHandler.cacheSchema(conn); //cache
                try {
                    com.amazonaws.athena.connectors.jdbc.kdb.KdbMetadataHandler.getKdbFunctionList(); //cache
                } catch(IOException ex) {
